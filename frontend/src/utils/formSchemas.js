...
export const maintenanceReportSchema = z.object({
    title: zodStringPipe(z.string().min(1, { message: 'Title is required' })),
    notes: zodStringPipe(z.string().or(z.null())),
    status: zodStringPipe(z.string({errorMap: () => ({message: 'Please select a status'})})),
    priority: zodStringPipe(z.string().or(z.null())),
    category: zodStringPipe(z.string().min(1, { message: 'Category is required' })),
    categoryOther: zodStringPipe(z.string().or(z.null())),
    pendingTagRequest: z.boolean().optional(),
    // reporterId removed: backend derives reporter from req.user
    unitId: zodNumberInputPipe(
        z.string({ errorMap: () => ({ message: 'Please select a unit' }) }).or(z.number())
    ),
